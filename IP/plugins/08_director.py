...
<style>
.director-panel {
    background: linear-gradient(135deg, #050505 0%, #121214 100%);
    border-radius: 8px;
    padding: 20px;
...
class DirectorIntegration:
    ...
    def _adapter_detect_stuck_patterns(
        ...
        if isinstance(productivity_score, (int, float)) and productivity_score < 0.25:
            suggestions.append(
                "Low productivity trend detected; suggest context reset."
            )
        ...
        if director.import_error:
            return mo.vstack(
                [
                    ...
                    mo.md("Director integration is unavailable in this environment."),
                    mo.md(f"`{director.import_error}`"),
                    mo.md(
