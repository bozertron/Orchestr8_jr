...
@dataclass
class PatchbayDryRunResult:
    """Dry-run result for a proposed patchbay rewire operation."""
    action: str
    source_file: str
    current_target: str
    proposed_target: str
    ...
    current_import_statement: Optional[str] = None
    proposed_import_statement: Optional[str] = None
    checks: Dict[str, bool] = field(default_factory=dict)
    issues: List[str] = field(default_factory=list)
    warnings: List[str] = field(default_factory=list)

    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary with camelCase keys for UI payload transport."""
        return {
            "action": self.action,
            "sourceFile": self.source_file,
            "currentTarget": self.current_target,
            "proposedTarget": self.proposed_target,
            "importType": self.import_type,
            "canApply": self.can_apply,
            "sourceLine": self.source_line,
            "currentImportStatement": self.current_import_statement,
            "proposedImportStatement": self.proposed_import_statement,
            "checks": self.checks,
            "issues": self.issues,
    ...
    def verify_project(
        ...
        if file_paths is None:
            ...
            default_exclusions = {
                ...
                "dist",
                "build",
                # Large/non-runtime trees that can stall Code City startup.
                "marimo",
                "vscode-marimo",
                ".taskmaster",
                ".planning",
            ...
            extra_exclusions = {
                item.strip()
                for item in os.getenv("ORCHESTR8_CONN_EXCLUDE_DIRS", "").split(",")
                if item.strip()
            }
            exclusions = default_exclusions | extra_exclusions

            max_files_raw = os.getenv("ORCHESTR8_CONN_MAX_FILES", "2500").strip()
            try:
                max_files = max(100, int(max_files_raw))
            except ValueError:
                max_files = 2500
...
def verify_all_connections(project_root: str, files_df) -> Tuple:
    """Compatibility wrapper for feature-sliced connection verification service."""
    from IP.features.connections.service import (
        verify_all_connections as _verify_all_connections,
    )

    return _verify_all_connections(project_root=project_root, files_df=files_df)
...
def build_connection_graph(project_root: str) -> ConnectionGraph:
    ...
    from IP.features.connections.service import (
        build_connection_graph as _build_connection_graph,
    )

    return _build_connection_graph(project_root)
...
def dry_run_patchbay_rewire(
    ...
    """Compatibility wrapper for feature-sliced patchbay dry-run validation."""
    from IP.features.connections.patchbay import (
        dry_run_patchbay_rewire as _dry_run_patchbay_rewire,
    )

    return _dry_run_patchbay_rewire(
...
def apply_patchbay_rewire(
    ...
    """Compatibility wrapper for feature-sliced patchbay apply workflow."""
    from IP.features.connections.patchbay import (
        apply_patchbay_rewire as _apply_patchbay_rewire,
    )

    return _apply_patchbay_rewire(
        project_root=project_root,
        source_file=source_file,
        current_target=current_target,
        proposed_target=proposed_target,
