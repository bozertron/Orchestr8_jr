...
| Implement three-color system | 🔄 In Progress | Gold/Purple/Blue only |
| Building footprint from file size | ⬜ Not Started | Bigger file = bigger building |
| Building height from export count | ⬜ Not Started | More exports = taller |
| Single building renders correctly | ⬜ Not Started | End-to-end test |

**Phase 0 Deliverable:**

```
INPUT:  One .ts or .vue file
OUTPUT: One Barradeau-style building where:
        - Footprint size ∝ file line count
        - Height ∝ number of exports
        - Color = gold (hardcoded for now, health checking comes later)
...
4. No panel, no rooms, no edges yet—just the building shell
...
## VII. Design Decisions (Resolved)

| Question | MVP Answer | Roadmap |
|----------|------------|---------|
| **Building layout** | Grid | "Making it pretty" - organic/force-directed layouts |
| **Edge routing** | Straight lines | "Intelligent physical routing to increase density" |
| **Scale limits** | No limit (VS Code feeds us) | N/A |
| **LLM integration** | Whatever is easiest for LLM to see and actuate clearly | Optimize based on what works |
| **Real-time vs snapshot** | However VS Code does it | Leverage existing debugging tools |
...
- Not rendered as buildings because they're infrastructure, not user code
...
│   ┌─────────┐  ┌─────────┐  ┌─────────┐                        │
│   │Building │  │Building │  │Building │     ┌────────────────┐ │
│   │handler  │  │connect  │  │ types   │     │  TOWN SQUARE   │ │
│   └─────────┘  └─────────┘  └─────────┘     │                │ │
│                                              │ tsconfig.json  │ │
│   ┌─────────┐  ┌─────────┐                  │ package.json   │ │
│   │Building │  │Building │                  │ .gitignore     │ │
│   │ utils   │  │ events  │                  │ [scrolling...] │ │
│   └─────────┘  └─────────┘                  └────────────────┘ │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
## VIII. Success Criteria (Full System)
