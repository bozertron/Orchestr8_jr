...
- Font stack migration (2026-02-13):
  ...
  - Current repo integrity check note:
    - `Font/HardCompn.ttf` currently matches `Font/mini_pixel-7.ttf` byte-for-byte in this workspace.
    - Runtime now skips HardCompn injection when it detects duplicate payload and falls back to remaining profile fonts.
    - A distinct HardCompn webfont file is still required for true deco headline rendering.
...
6. Saved HTML snapshots can look live but are not callback-equivalent to `marimo run`:
   - Opening downloaded `orchestr8(...).html` from disk can render UI shells while Python callbacks remain non-functional.
   - Button/state behavior requires an active kernel/WebSocket session.
